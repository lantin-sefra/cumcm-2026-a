...
% === MH-DENSE-LAYOUT：紧密排版但不压正文 ===
% 让图表放不下时后续文字继续排版，浮动体就近转移到页顶/页底；
% 只收紧浮动体、公式与标题的垂直空白，不改正文字号、页边距和正文行距。
...
\renewcommand\section{\@startsection{section}{1}{\z@}%
  {-2.9ex \@plus -.8ex \@minus -.2ex}%
  {1.7ex \@plus .2ex}%
  {\centering\normalfont\Large\bfseries}}
\renewcommand\subsection{\@startsection{subsection}{2}{\z@}%
  {-2.45ex\@plus -.8ex \@minus -.2ex}%
  {1.05ex \@plus .2ex}%
  {\normalfont\large\bfseries}}
\renewcommand\subsubsection{\@startsection{subsubsection}{3}{\z@}%
  {-2.35ex\@plus -.7ex \@minus -.2ex}%
  {1.0ex \@plus .2ex}%
  {\normalfont\normalsize\bfseries}}
\makeatother
\AtBeginDocument{%
...
% 参考文献保持正文字号，但采用更紧凑的单倍型行距；不改文献内容。
...
$\eta=(r/R)^2$），计算域固定且方程不再含表观对流。正式配置 $N=1280$、$\Delta t=0.25$~s，
物质坐标主模型求得\textbf{时长 $51.0869$~h}、末态半径 $1.2000$~cm；
实验室系 Landau（保留对流）交叉验证为 $52.6476$~h，两者相差 $1.5607$~h（$3.0550\%$）。

四问质量收支残差均不超过 $1.6\times10^{-14}$；扩散系数前因子是最强不确定度来源。
忽略潜热会使时长预测偏短（约 $10\%$）。
...
% === AI 工具使用声明（自动插入）===
% 正文浮动体先就近排完，但不强制另起空白页。
\FloatBarrier
\input{sections/Z_ai_disclosure}
% AI 声明后若本页仍有空间，让参考文献直接接排，不人为留下页尾空白。
\nopagebreak[4]
