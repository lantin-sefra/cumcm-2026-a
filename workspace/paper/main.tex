...
至 $1800$~s，得中心 $33.577$~℃ 与 $2.5500$~kg/kg、表面 $36.786$~℃ 与 $1.511$~kg/kg。
...
系数解含水率，最多 $3$ 次收敛。$3$~h 时中心已升至 $49.85$~℃ 接近烘房温度，中心含水率
仍为 \textbf{$1.766$~kg/kg}、表面 $1.008$~kg/kg，表明温度场先达准稳态，其后由内部扩散
阻力控制。

针对问题三，定义达标事件 $t_{\rm end}=\min\{t:\max_r C(r,t)<0.15\}$，取逐点最大值而非
体平均（后者会在芯部尚未达标时提前跨过阈值），控制点为含水率最高的轴心，
并在输出亚步间线性插值定位。
求得\textbf{烘干时长 $56.706$~h}，达标时各位置含水率由中心 $0.1500$ 递降至表面 $0.0525$；
干燥速率自 $0.6899$~kg/(kg$\cdot$h) 起单调衰减，全程无恒速段。

针对问题四，外边界随失水内移。附件 2 全部 $145$ 个半径数据经单调化与 PCHIP 保形插值给出
$R(t)$。干基含水率跟随固体骨架，故以物质坐标 $\eta=(r/R(t))^2$ 为主实现，收缩进入
$4/R^2$ 几何因子，固相速度由均匀收缩唯一确定为 $v_s=r\dot R/R$。Eulerian Landau 变换
作为交叉验证，二者相差 $2.78\%$。求得\textbf{时长 $50.780$~h}、末态半径 $1.20$~cm，
较同物性尺寸冻结情形缩短 $60.6\%$。补齐「原物性+收缩」后的 $2\times2$ 因子设计给出
交互项 $-46.240$~h，与主效应同量级，故物性贡献与收缩贡献不能作唯一加性分解；
Shapley 平均分别为 $+48.812$~h 与 $-55.006$~h。反演的体积收缩系数变异系数 $22\%$，
线性收缩律不成立。

四问离散水分方程收支残差均不超过 $7\times10^{-14}$；扩散系数前因子是最强不确定度来源，忽略蒸发潜热
使时长预测偏短约一成。
