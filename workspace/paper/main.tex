...
对流项，以一阶迎风保证对角占优。求得\textbf{时长 $52.190$~h}、末态半径 $1.20$~cm。同一
离散与判据下隔离两效应得加性分解：净差 $-4.519$~h $=$ 换物性 $+71.885$~h $+$ 加收缩
$-76.404$~h，残差 $7.1\times10^{-15}$~h，两效应各约为净差的十六倍而近乎相消。

四问质量收支残差均不超过 $1.6\times10^{-14}$；扩散系数前因子是最强不确定度来源，忽略蒸发潜热
使时长预测偏短约一成。
