...
\begin{longtable}{lp{5.5cm}p{5.7cm}}
  ...
  程序入口 & \verb|main.py|          & 四问求解编排与结果写出 \\
           ...
           & \verb|validate.py|      & 历史粗网格诊断（非正式结果，见 \path{output/legacy/}） \\
  ...
  计算结果 & \verb|result1.xlsx|     & 问题一温度与水分浓度全网格结果 \\
           & \verb|result2.xlsx|     & 问题二前 $3$~h（$1\sim10800$~s）温度与含水率全网格 \\
           & \verb|result3.xlsx|     & 问题三 $0\to t_{\rm end}$ 水分浓度全网格 \\
           & \verb|result4.xlsx|     & 问题四物质坐标主模型（$\chi=1$）收缩域水分浓度全网格 \\
           ...
           & \path{legacy/validation.json} & 历史粗网格诊断（非正式结果） \\
\end{longtable}

\section{程序源代码}

下列代码为完整可运行源程序，按依赖层次排列：先给出参数与数据读入，再给出离散核与物性、
时间推进与求解编排，最后给出四问的调用与验证。运行方式为在 \verb|workspace/| 目录下执行
\verb|python code/main.py|，程序自动读入附件并把全部报表与工作簿写入 \verb|output/|。
旧 \verb|validate.py| 不再随正式流程自动执行。
...
\subsection{历史粗网格诊断：validate.py}
下列源码为仓库中真实存在的 \verb|validate.py|（LEGACY DIAGNOSTIC，非正式结果）。
