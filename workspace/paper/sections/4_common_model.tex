...
按第三类边界建模而不用第一类，理由见问题一的分析\upcite{Yilbas2003}。
轴心处由几何对称性给出零通量条件
...
空气温度自 $28~^\circ$C 快速抬升后进入平台，含湿量同步趋于稳定。稳态外推段使
问题二至问题四的长时程求解具有明确的边界驱动：进入该段后药材内部的继续变化只由内部
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth,height=0.8\textheight,keepaspectratio]{../figures/tikz_cylinder_cv.pdf}
...
新时间层三个结点同时未知、旧层只有同一结点进入右端项，故系数矩阵每行恰有三个非零元。
...
以消除分裂本身引入的滞后\upcite{Fromme1997}。
