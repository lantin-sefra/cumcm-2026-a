...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.74\textwidth,height=0.30\textheight,keepaspectratio]{../figures/fig_input_chamber.pdf}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.74\textwidth,height=0.30\textheight,keepaspectratio]{../figures/fig_property_maps.pdf}
...
$\beta$ 取 $h$（温度，守恒形）或 $h_m$（含水率）；轴心控制体则由式~\eqref{eq:bc_center} 的
对称条件关闭，不需要虚拟结点。
...
\kappa_i V_i\,\frac{\varphi_i^{n+1}-\varphi_i^{n}}{\Delta t}
...
其中容量系数 $\kappa$ 在温度方程取 $\rho c_p$、在含水率方程取 $1$，$\Gamma$ 为对应的输运系数（$k$ 或 $D$）。显式格式受 $\Delta t\le\Delta r^2/(2\alpha)$ 限制，
